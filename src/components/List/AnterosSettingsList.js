...
export class AnterosSettingsList extends Component {
    ...
    if(group.header){
      return (
        <View key={'group_' + index}>
          <AnterosText style={[{margin:5},group.header.headerStyle]} numberOfLines={group.header.headerNumberOfLines} ellipsizeMode="tail" ref={group.header.headerRef}>{group.header.headerText}</AnterosText>
  ...
  itemArrowIcon(item) {
    ...
    if(item.hasNavArrow){
        return <AnterosImage style={[styles.rightSide, item.arrowStyle]} source={ARROW_ICON} />;
    }
 ...
 */
AnterosSettingsList.Header = createReactClass({
  propTypes: {
 ...
 */
AnterosSettingsList.Item = createReactClass({
  propTypes: {
    /**
