...
export default class AnterosSwiper extends Component {
  ...
  initState(props, updateIndex = false) {
    ...
    initState.dir = props.horizontal === false
      ? 'y'
      : 'x'
  ...
  onLayout = (event) => {
    ...
    // contentOffset is not working in react 0.48.x so we need to use scrollTo to
    // emulate offset.
    if (Platform.OS === 'ios') {
  ...
  loopJump = () => {
    if (!this.state.loopJump) 
      return
    const i = this.state.index + (this.props.loop
      ? 1
      : 0)
   ...
   */
  autoplay = () => {
    if (!Array.isArray(this.props.children) || !this.props.autoplay || this.internals.isScrolling || this.state.autoplayEnd) 
      return
    ...
    this.autoplayTimer = setTimeout(() => {
      ...
      this.scrollBy(this.props.autoplayDirection
        ? 1
        : -1)
  ...
  onScrollBegin = e => {
    ...
    this.props.onScrollBeginDrag && this
      .props
  ...
  onScrollEnd = e => {
    ...
    this.updateIndex(e.nativeEvent.contentOffset, this.state.dir, () => {
      ...
      this.props.onMomentumScrollEnd && this
        .props
  ...
  onScrollEndDrag = e => {
    ...
    const previousOffset = horizontal
      ? offset.x
      : offset.y
  ...
  updateIndex = (offset, dir, cb) => {
    ...
    // the variation of `index` more than 1. parseInt() ensures it's always an
    // integer
    ...
    if (loopJump) {
      // when swiping to the beginning of a looping set for the third time, the new
      // offset will be the same as the last one set in state. Setting the offset to
      // the same thing will not do anything, so we increment it by 1 then immediately
      // set it to what it should be, after render.
  ...
  scrollBy = (index, animated = true) => {
    if (this.internals.isScrolling || this.state.total < 2) 
      return
    const state = this.state
    const diff = (this.props.loop
      ? 1
      : 0) + index + this.state.index
    let x = 0
    let y = 0
    if (state.dir === 'x') 
      x = diff * state.width
    ...
    if (Platform.OS !== 'ios') {
      this.scrollView && this.scrollView[animated
          ? 'setPage'
  ...
  renderPagination = () => {
    ...
    return (
      <View
        pointerEvents='none'
        style={[
        AnterosTheme['swiperPagination_' + this.state.dir],
  ...
  renderTitle = () => {
    ...
    return title
      ? (
        <View style={AnterosTheme.swiperTitle}>
  ...
  renderNextButton = () => {
    ...
    if (this.props.loop || this.state.index !== this.state.total - 1) {
      button = this.props.nextButton || <Text style={AnterosTheme.swiperButtonText}>›</Text>
    }
  ...
  renderPrevButton = () => {
    ...
    if (this.props.loop || this.state.index !== 0) {
      button = this.props.prevButton || <Text style={AnterosTheme.swiperButtonText}>‹</Text>
    }
  ...
  renderButtons = () => {
    return (
      <View
        ...
        style={[
        AnterosTheme.swiperButtonWrapper, {
  ...
  renderScrollView = pages => {
    if (Platform.OS === 'ios') {
      return (
        <ScrollView
          ...
          contentContainerStyle={[AnterosTheme.swiperWrapperIOS, this.props.style]}
    ...
    return (
      <ViewPagerAndroid
        ...
        key={pages.length}
        style={[AnterosTheme.swiperWrapperAndroid, this.props.style]}>
        {pages}
      </ViewPagerAndroid>
    )
  }
  ...
  render() {
    ...
    return (
      <View
        style={[AnterosTheme.swiperContainer, containerStyle]}
