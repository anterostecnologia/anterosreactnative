...
export default class AnterosWheel extends Component {
  ...
  buildProps() {
    ...
    style = [{
      backgroundColor: AnterosTheme.wheelColor,
      overflow: 'hidden',
    }].concat(style);
    itemStyle = [{
      backgroundColor: 'rgba(0, 0, 0, 0)',
      fontSize: AnterosTheme.wheelFontSize,
      color: AnterosTheme.wheelTextColor,
    }].concat(itemStyle);
    holeStyle = [{
      backgroundColor: 'rgba(0, 0, 0, 0)',
      height: AnterosTheme.wheelHoleHeight,
      zIndex: 1,
    }].concat(holeStyle);
    maskStyle = [{
      backgroundColor: AnterosTheme.wheelMaskColor,
      opacity: AnterosTheme.wheelMaskOpacity,
      flex: 1,
      zIndex: 100,
    }].concat(maskStyle);
    if (holeLine === undefined) {
      holeLine = <View style={{height: AnterosTheme.wheelHoleLineWidth, backgroundColor: AnterosTheme.wheelHoleLineColor}} />;
    } else if (typeof holeLine === 'number') {
      holeLine = <View style={{height: holeLine, backgroundColor: AnterosTheme.wheelHoleLineColor}} />;
    }
