...
export class AnterosTransformView extends Component {
    ...
    this.handleTouches(e.nativeEvent.touches, (dx, dy, scaleRate) => {
      ...
      if (tension) {
        if (x > this.initContentLayout.x) 
          dx /= 3;
        else if ((x + width) < (this.initContentLayout.x + this.initContentLayout.width)) 
          dx /= 3;
        if (y > this.initContentLayout.y) 
          dy /= 3;
        else if ((y + height) < (this.initContentLayout.y + this.initContentLayout.height)) 
          dy /= 3;
        }
      this.dxSum += dx;
      this.dySum += dy;
  ...
  handleTouches(touches, onHandleCompleted) {
    ...
    if (touches.length == 1) {
      t0 = {
        x: prevTouches[0].pageX,
        y: prevTouches[0].pageY
      };
      t1 = {
        x: touches[0].pageX,
        y: touches[0].pageY
    ...
    if (distance0 && distance1) {
      ...
      // unused code let {x, y, width, height} = this.contentLayout; let scalePointX =
      // (prevTouches[1].locationX + prevTouches[0].locationX) / 2 - x; let
      // scalePointY = (prevTouches[1].locationY + prevTouches[0].locationY) / 2 - y;
      // //view center point position let centerX = width / 2; let centerY = height /
      // 2; //the scale point with the center of the view as the origin let
      // spBeforScaleX = scalePointX - centerX; let spBeforScaleY = scalePointY -
      // centerY; let spAfterScaleX = spBeforScaleX * scaleRate; let spAfterScaleY =
      // spBeforScaleY * scaleRate; //So that the scale point does not seem to move dx
      // += spBeforScaleX - spAfterScaleX; dy += spBeforScaleY - spAfterScaleY;
  ...
  handleRelease() {
    ...
    let animates = [];
    newX !== null && animates.push(Animated.spring(translateX, {
      toValue: newX,
      friction: 9
    }));
    newY !== null && animates.push(Animated.spring(translateY, {
      toValue: newY,
      friction: 9
    }));
    newScale !== null && animates.push(Animated.spring(scale, {
      toValue: newScale,
      friction: 9
    }));
    if (animates.length > 0) {
      ...
      let canDoMagnetic = !onWillMagnetic || onWillMagnetic(translateX._value, translateY._value, scale._value, newX, newY, newScale,);
      canDoMagnetic && Animated
        .parallel(animates)
        .start(e => {
