...
const generatePanHandlers = (onStart, onMove, onRelease) =>
    PanResponder.create({
...
function fetchImageSize(images= []) {
...
const getImagesWithoutSize = (images) =>
    images.filter(({width, height}) => !width || !height);

const scalesAreEqual = (scaleA, scaleB) =>
    Math.abs(scaleA - scaleB) < SCALE_EPSILON;

export class AnterosImageViewer extends Component {
    constructor(props) {
        super(props);
    ...
    onNextImagesReceived(images, imageIndex = 0) {
...
AnterosImageViewer.defaultProps = {
    images: [],
    imageIndex: 0,
    onClose: () => {},
    renderFooter: () => {}
