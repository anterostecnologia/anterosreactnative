...
export class AnterosElasticStack extends Component {
  static propTypes = {
  ...
  renderElastickItems() {
    ...
    return Array.from({ length: elastickItemsCount }).map((_, i) => {
      const itemIndex = AnterosElasticStack.calculateNextItemIndex(
        itemsLength,
  ...
  setItemIndex = (activeItemIndex, onSwipedToDirection, isStackEnded) => {
    ...
    const prevItemIndex = AnterosElasticStack.calculatePreviousItemIndex(
