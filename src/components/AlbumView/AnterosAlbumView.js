...
export default class AnterosAlbumView extends Component {
  ...
  static propTypes = {
    ...ViewPropTypes,
    images: PropTypes
      .arrayOf(PropTypes.oneOfType([Image.propTypes.source, PropTypes.element]))
      .isRequired,
  ...
  static Sheet = AnterosAlbumSheet;
  ...
  changeIndex(newIndex) {
    ...
    Animated.parallel(this.animateActions.map((item, index) => Animated.spring(item.variable, {
  ...
  onWillMagnetic(translateX, translateY, scale, newX, newY, newScale) {
    ...
    if (scale < 1) {
      return true;
    } else if ((ltx < triggerWidth && rtx > -triggerWidth) || (ltx >= triggerWidth && index === 0) || (rtx <= -triggerWidth && index === images.length - 1)) {
      index > 0 && this
        .refs['sheet' + (index - 1)]
        .scrollX(0, true);
      index < (images.length - 1) && this
    ...
    this.changeIndex(ltx >= triggerWidth
      ? index - 1
      : index + 1);
  ...
  renderImage(index) {
    ...
    return (<AnterosAlbumSheet
      ...
      pointerEvents={index === this.state.index
      ? 'auto'
      : 'none'}
      minScale={1}
      maxScale={maxScale}
      ...
      tension={false}
      image={images[index]}
      thumb={thumbs instanceof Array && thumbs.length > index
      ? thumbs[index]
      : null}
  ...
  render() {
    ...
    if (React.isValidElement(control)) {
      ...
      control = <this.constructor.Control
        style={{
        paddingBottom: AnterosTheme.screenInset.bottom
      }}
        index={this.state.index}
        total={images.length}
        carousel={this}/>
