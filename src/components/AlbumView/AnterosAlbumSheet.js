...
export class AnterosAlbumSheet extends AnterosTransformView {
  ...
  static propTypes = {
    ...
    image: PropTypes
      .oneOfType([Image.propTypes.source, PropTypes.element])
      .isRequired,
  ...
  static defaultProps = {
    ...AnterosTransformView.defaultProps,
    maxScale: 3,
    minScale: 1,
  ...
  getImageSize(source, success, failure) {
    if (typeof source === 'number') {
      ...
      Image.getSize(source.uri, (width, height) => success && success(width, height), (error) => failure && failure(error));
    } else {
  ...
  getElementSize(element) {
    ...
    if (React.isValidElement(element)) {
      ...
      if (style.width === null || style.width === undefined || style.height === null || style.height === undefined) {
  ...
  getScrollValue() {
    ...
    let exceedWidth = scaleWidth > viewWidth
    ...
    return {leftX, rightX, centerLeftX, centerRightX};
  }
