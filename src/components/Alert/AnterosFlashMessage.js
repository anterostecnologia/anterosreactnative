...
import { StyleSheet, TouchableWithoutFeedback, StatusBar, Platform, Dimensions, Animated, Image, Text, View } from "react-native";
...
export function showMessage(...args) {
  const ref = AnterosFlashMessageManager.getDefault();
  if (!!ref) {
...
export const renderFlashMessageIcon = (icon = "success", style = {}, customProps = {}) => {
  switch (icon) {
    case "success":
      return (
        <AnterosImage style={[styles.flashIcon, style]} source={require("../../assets/images/fm_icon_success.png")} {...customProps} />
      );
    case "info":
      return <AnterosImage style={[styles.flashIcon, style]} source={require("../../assets/images/fm_icon_info.png")} {...customProps} />;
    case "warning":
      return (
        <AnterosImage style={[styles.flashIcon, style]} source={require("../../assets/images/fm_icon_warning.png")} {...customProps} />
      );
    case "danger":
      return (
        <AnterosImage style={[styles.flashIcon, style]} source={require("../../assets/images/fm_icon_danger.png")} {...customProps} />
      );
    default:
      return null;
  }
...
/**
 * Default MessageComponent used in AnterosFlashMessage
 * This component it's wrapped in `AnterosFlashMessageWrapper` to handle orientation change and extra inset padding in special devices
...
export const DefaultFlash = ({
  ...
  return (
    <AnterosFlashMessageWrapper position={typeof position === "string" ? position : null}>
      {wrapperInset => (
        <View
          style={styleWithInset(
            [
              ...
              !!message.backgroundColor
                ...
                : !!message.type &&
                  !!AnterosFlashMessage.ColorTheme[message.type] && {
                    backgroundColor: AnterosFlashMessage.ColorTheme[message.type],
                  },
              style,
            ],
            wrapperInset,
          ...
          <View style={styles.flashLabel}>
            ...
            {hasDescription && (
              <AnterosText style={[styles.flashText, !!message.color && { color: message.color }, textStyle]}>
                {message.description}
              </AnterosText>
            )}
          </View>
 ...
 * ```
 */
export class AnterosFlashMessage extends Component {
  static defaultProps = {
    /**
  ...
  static setColorTheme = theme => {
    AnterosFlashMessage.ColorTheme = Object.assign(AnterosFlashMessage.ColorTheme, theme);
  };
  constructor(props) {
    super(props);
  ...
  componentWillUnmount() {
    if (this.props.canRegisterAsDefault) {
      AnterosFlashMessageManager.unregister(this);
    }
  }
  /**
   * Non-public method
