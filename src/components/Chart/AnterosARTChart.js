...
import AmimatedCirclesMarker from './animated-circles-marker';
import {complement, makeBars3DChartPath, makeBarsChartPath, makeAreaChartPath, makeLineChartPath, makeSplineChartPath, makeCandlestickChartPath, makeCandlestickChart, interpolateColorsFixedAlpha, makeSpline, computeSplineControlPoints, makeCircle, getMinMaxValues, getMinMaxValuesCandlestick, getMinMaxValuesRange, getMaxSumStack, getMaxSumBars3d, findRectangleIndexContainingPoint, findClosestPointIndexWithinRadius, makeAreaRangeChartPath, makeLineStepChartPath, makeStackedBarsChartPath} from './util';
...
class AnterosARTChart extends Component {
    ...
    this.props.data.forEach(chart => {
      if (chart.type === 'bars' || chart.type.slice(-4) === 'area' || chart.type.slice(0,4) === 'area') {
        chart.data.forEach(d => {
           d.fillColors = {
            active: interpolateColorsFixedAlpha(chart.highCol || chart.lineColor || 'white',
                                                  chart.lowCol || chart.lineColor || 'white',
                                                  d.value/this.maxValue, 1),
            inactive: interpolateColorsFixedAlpha(chart.highCol || chart.lineColor || 'white',
                                                  chart.lowCol || chart.lineColor || 'white',
