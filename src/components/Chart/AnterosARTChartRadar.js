...
import {polarToCartesian, computeChartSum, makeCircle, findClosestPointIndexWithinRadius, complement} from './util';
...
class AnterosARTChartRadar extends Component {
  constructor(props) {
    super(props);
    this.state = {
      t: 0,
    ...
    let texts = this.props.labels.map((label, j) => {
      let coords = polarToCartesian(this.center, this.center, this.center + (this.props.labelsTextSize*.25 || 12*.25), this.angleStepSize*j + this.angleStepSize/2);
       return (<Text 
            ...
            //transform={{rotate: (this.angleStepSize*j + this.angleStepSize/2)+'deg'}}
            alignment="center"
            x={coords.x} 
            y={coords.y + PADDING_TOP}>
              {label}
